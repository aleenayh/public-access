...
export function CharacterCreateModal({
  ...
  const hasCreatedCharacter =
    gameState.players.find((player) => player.id === user.id)?.character !==
    null;
  const existsSomeKeeper = gameState.players.some(
    (player) => player.role === PlayerRole.KEEPER,
  );
  const [isOpen, setIsOpen] = useState(
    !hasCreatedCharacter && user.role === PlayerRole.PLAYER,
  );
  const [step, setStep] = useState<"initial" | "create" | "moves" | "confirm">(
    "initial",
  );
  ...
  const confirmCharacter = (data: CharacterCreateInputs) => {
    ...
    const character = {
      ...
      look,
      image: imageURL
        ? { type: "custom" as const, url: imageURL }
        : getUnusedFallback(gameState),
      abilities,
      takesYouBack,
      moves: [moves],
      ...
      conditions: ["", "", ""],
      cornerOfTheHouse: [
        ...(moveAdjustments?.cornerOfTheHouse || []),
        { marked: false, item: "" },
        { marked: false, item: "" },
      ],
      advancements: {
        ...
        <Dialog.Content className="DialogContent">
          ...
          {step === "initial" && (
            <div className="flex flex-col gap-4 text-center max-w-1/2 mx-auto">
              ...
              {!existsSomeKeeper && (
                <p>This game also doesn't have a Keeper yet.</p>
              )}
              <button
                type="button"
              ...
              <p className="text-xs text-theme-text-muted">
                Think this is a mistake? If you joined the game previously, make
                sure you joined with the same player name as before. Your
                current player name is {user.name}.
              </p>
            </div>
          )}
          {step !== "initial" && (
            <form onSubmit={handleSubmit(confirmCharacter)}>
              {step === "create" && (
                <div className="flex flex-wrap text-center w-full">
                  <BlendedInputWithWordCloud
                    optionSets={[nameOptions, surnameOptions]}
                    setValue={setValue}
                    register={register}
                    title="Name"
                    fieldName="name"
                  ...
                  <Divider />
                  <InputWithWordCloud
                    options={takesYouBackOptions}
                    setValue={setValue}
                    register={register}
                    title="What Takes You Back?"
                    pickNum={3}
                    fieldName="takesYouBack"
                  />
                  <Divider />
                  <div className="w-full flex flex-col">
                    <label
                      htmlFor="image"
                      className="text-xs text-theme-text-muted/80 text-left italic"
                    >
                    ...
                    <input
                      ...
                      className="w-full"
                    />
                    <p className="text-xs text-theme-text-muted/80 text-left">
                      paste image url. png, jpeg, webp accepted. displays in
                      square aspect ratio.
                    </p>
                  </div>
                  <Divider />
                  ...
                  <div className="flex gap-2 justify-center">
                    <div className="flex flex-col">
                      <label
                        htmlFor="abilities.vitality"
                        className="text-xs text-theme-text-muted/80 text-center"
                      >
                        Vitality
                      </label>
                      <input
                        type="number"
                        {...register("abilities.vitality")}
                    ...
                    <div className="flex flex-col">
                      ...
                      <input
                        type="number"
                        {...register("abilities.sensitivity")}
                        className="w-full md:w-1/2 mx-auto text-center"
                        min={-3}
                      />
                    </div>
                  ...
                  <button
                    ...
                    className="formButton mx-auto my-6"
                  >
                    Choose Latchkey Move
                  </button>
                </div>
              )}
              {step === "moves" && (
                ...
                <>
                  <div className="flex flex-col gap-2 w-full">
                    <p className="text-sm text-theme-text-muted">
                      Please review your character and confirm your choices.
                    </p>
                  ...
                  <div className="flex w-full justify-evenly">
                    ...
                    <button type="submit" className="formButton mx-auto my-6">
                      Confirm
                    </button>
                  </div>
                </>
    ...
    <div className="flex flex-col gap-2 items-center justify-start">
      <div className="flex gap-2 w-full justify-start">
        {image ? (
          <img
            src={image}
            className="max-h-32 aspect-square object-contain flex-1 mx-auto"
          />
        ) : (
          <div className="flex-1 flex justify-center">
            <VisualThumbnailOnly imageKey={getUnusedFallback(gameState).icon} />
          </div>
        )}
      ...
      <div className="flex gap-2">
        <p className="flex-1">
          <span className="[font-variant:small-caps] text-theme-text-muted pr-2">
            Look:
          </span>
          {look}
        </p>
      ...
      <p>
        <span className="[font-variant:small-caps] text-theme-text-muted flex w-full">
          Moves:
        </span>
        <span className="bold text-theme-text-accent pr-2">{moves.name}:</span>
        <span className="text-sm leading-none">{moves.description}</span>
      </p>
      <div className="flex gap-2 justify-center">
        <div className="flex flex-col">
          <label
...
function BlendedInputWithWordCloud({
  optionSets,
  setValue,
  register,
  title,
  ...
  optionSets: string[][];
  setValue: UseFormSetValue<CharacterCreateInputs>;
  register: UseFormRegister<CharacterCreateInputs>;
  title: string;
  fieldName: keyof CharacterCreateInputs;
}) {
  ...
  return (
    <div className="flex flex-col gap-2 w-full">
      ...
      <div className="w-full">
        <p className="text-xs text-theme-text-muted/80 text-left">
          <strong>Pick one of each, or make up your own:</strong>
        </p>
        <div className="flex gap-2 flex-col md:flex-row">
          {optionSets.map((options, setIndex) => {
            const selectedOpt =
              setIndex === 0 ? selectedFirstName : selectedLastName;
              ...
              >
                ...
                {options.map((opt) => (
                  <button
                    type="button"
                    key={opt}
                    className={`wordCloudButton ${selectedOpt === opt ? "border border-theme-border" : ""}`}
                    onClick={() => handleClickCloud(setIndex, opt)}
                  >
                    {opt}
                  </button>
                ))}
...
function InputWithWordCloud({
  ...
  return (
    <div className="w-full flex flex-col">
      ...
      <input type="text" {...register(fieldName)} className="w-full" />
      <p className="text-xs text-theme-text-muted/80 text-left">
        <strong>Pick {pickNum} or make up your own:</strong>
      </p>
      <div className="flex flex-wrap gap-1 text-xs">
        {options.map((opt) => (
          <button
            type="button"
            key={opt}
            className={`wordCloudButton ${selectedOptions.includes(opt) ? "border border-theme-border" : ""}`}
            onClick={() => handleClickCloud(opt)}
          >
            {opt}
          </button>
        ))}
...
function MovePicker({
  ...
  onContinue: (selectedMove: string) => void;
  onBack: () => void;
}) {
  const { gameState } = useGame();
  const movesInPlay = gameState.players
    .flatMap((player) => player.character?.moves || [])
    .map((move) => move.name);
        ...
        {Object.entries(moves).map(([name, description]) => {
          ...
          return (
            <button
              type="button"
              key={`move-${name}`}
              onClick={() => setSelectedMove(name)}
              className={`border border-theme-border box-border rounded-md p-2 h-fit bg-theme-bg-primary max-h-64 overflow-y-auto ${selectedMove === name ? "border-theme-border-accent border-2 shadow-lg bg-theme-bg-secondary" : ""}`}
            >
              <h4
                className={`text-theme-text-accent ${selectedMove === name ? "brightness-125" : ""}`}
              >
                {name}
              </h4>
